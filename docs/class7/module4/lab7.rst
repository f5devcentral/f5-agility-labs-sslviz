Conclusion
=============================

To conclude this lab, we have successfully built new SSLO Services using Ansible automation playbooks.  

We demonstrated to ability to dynamically modify configuration objects within SSL Orchestrator. This is a critical capability to have when you are looking to operationalize your forward proxy deployment with various services to scale and adapt to the needs of your environment.

The possibilities are limitless to what you can do with this capability with the eye of a security engineer/architect. You can build out various services and service chains to steer traffic to for different types of inspection, and use the TAP service to gain visibility into the traffic flowing through those services to gain insights into client behaviors, security posture, and more.